...
class Admin extends Controller{
    ...
    public function getGroupTheoryStatistics($name){
        ...
        for($i = 0; $i < count($categories); $i++){
            ...
            for($j = 0; $j < count($topics); $j++){
                $seen = DB::select('SELECT COUNT(*) as many FROM glance_student GS, glances G WHERE GS.glance_id = G.id and G.type = ? and GS.student_id = (select id from students where group_id = ?);', [ 'T', $group -> id]);
                $seen = $seen[0] -> many;
                $people[$i][$j] = $seen;
                $visualizations += $seen;
    ...
    public function getGroupQuestionnaireStatistics($name){
        ...
        for($i = 0; $i < count($categories); $i++){
            ...
            $users_in_group_count  = count($users_in_group);
            for($j = 0; $j < count($topics); $j++){
                $seen = DB::select('SELECT COUNT(*) as many FROM glance_student GS, glances G WHERE GS.glance_id = G.id and G.type = ? and GS.student_id = (select id from students where group_id = ?);', [ 'C', $group -> id]);
                $seen = $seen[0] -> many;
                $people[$i][$j] = $seen;
                $visualizations += $seen;
                $percentages[$i][$j] = $users_in_group_count > 0 ? $seen * 100 / $users_in_group_count : 0;
    ...
    public function getGroupSimulationStatistics($name){
        ...
        for($i = 0; $i < count($categories); $i++){
            ...
            $users_in_group_count  = count($users_in_group);
            for($j = 0; $j < count($topics); $j++){
                $seen = DB::select('SELECT COUNT(*) as many FROM glance_student GS, glances G WHERE GS.glance_id = G.id and G.type = ? and GS.student_id = (select id from students where group_id = ?);', [ 'S', $group -> id]);
                $seen = $seen[0] -> many;
                $people[$i][$j] = $seen;
                $visualizations += $seen;
                $percentages[$i][$j] = $users_in_group_count > 0 ? $seen * 100 / $users_in_group_count : 0;
    ...
    public function getSchoolTheoryStatistics($name){
        ...
        for($i = 0; $i < count($categories); $i++){
            ...
            for($j = 0; $j < count($topics); $j++){
                $seen = DB::select('SELECT COUNT(*) as many FROM glance_student GS, glances G WHERE GS.glance_id = G.id and G.type = ? and GS.student_id = (select id from students where school_id = ?);', ['T', $school -> id]);
                $seen = $seen[0] -> many;
                $people[$i][$j] = $seen;
                $visualizations += $seen;
    ...
    public function getSchoolQuestionnaireStatistics($name){
        ...
        for($i = 0; $i < count($categories); $i++){
            ...
            $users_in_school_count  = count($users_in_school);
            for($j = 0; $j < count($topics); $j++){
                $seen = DB::select('SELECT COUNT(*) as many FROM glance_student GS, glances G WHERE GS.glance_id = G.id and G.type = ? and GS.student_id = (select id from students where school_id = ?);', ['Q', $school -> id]);
                $seen = $seen[0] -> many;
                $people[$i][$j] = $seen;
                $visualizations += $seen;
                $percentages[$i][$j] = $users_in_school_count > 0 ? $seen * 100 / $users_in_school_count : 0;
    ...
    public function getSchoolSimulationStatistics($name){
        ...
        for($i = 0; $i < count($categories); $i++){
            ...
            $users_in_school_count  = count($users_in_school);
            for($j = 0; $j < count($topics); $j++){
                $seen = DB::select('SELECT COUNT(*) as many FROM glance_student GS, glances G WHERE GS.glance_id = G.id and G.type = ? and GS.student_id = (select id from students where school_id = ?);', ['S', $school -> id]);
                $seen = $seen[0] -> many;
                $people[$i][$j] = $seen;
                $visualizations += $seen;
                $percentages[$i][$j] = $users_in_school_count > 0 ? $seen * 100 / $users_in_school_count : 0;
